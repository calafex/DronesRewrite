...
function ENT:Initialize()
	...
	self:AddWeapon("Swiborg eyes", {
		...
		Attack = function(self, gun)
			...
			local ef = EffectData()
			ef:SetStart(Vector(15, -10.5, 4.5))
			...
			if CurTime() > gun.NextShoot then
				...
				for k, ent in pairs(ents.FindInSphere(tr.HitPos, 200)) do
					ent:Ignite(2)

					local phys = ent:GetPhysicsObject()
					if IsValid(phys) then phys:ApplyForceCenter(VectorRand() * 300 + vector_up * 600) end
				end

				util.BlastDamage(self, IsValid(self:GetDriver()) and self:GetDriver() or self, tr.HitPos, 200, 1337)
